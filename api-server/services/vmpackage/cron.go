...
// ListDiscoveryDatasourcesForAccount is ListDiscoveryDatasources scoped to
// datasources whose discovery target is targetAccountID — the SQL-level
// equivalent of filtering the full list by TargetAccountID, used by the
// manual account-wide scan trigger so a single account's scan request
// doesn't have to list every tenant's datasources first.
func ListDiscoveryDatasourcesForAccount(dbms *database.DatabaseManager, targetAccountID string) ([]DiscoveryDatasource, error) {
	var rows []struct {
		IntegrationID   string         `db:"integration_id"`
		TenantID        string         `db:"tenant_id"`
		AccountID       string         `db:"account_id"`
	...
	err := dbms.Db.Select(&rows, `
		...
		WHERE i.type = 'discovery' AND i.status = 'enabled' AND target.cloud_account_id = $1`, targetAccountID)
	if err != nil {
		return nil, fmt.Errorf("vmpackage: list discovery datasources for account %s: %w", targetAccountID, err)
	}
