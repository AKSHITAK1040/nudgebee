...
func TestNotebookTool_Schema(t *testing.T) {
	...
	assert.Contains(t, schema.Properties, notebookAppendArg)
	assert.Equal(t, core.ToolSchemaTypeBoolean, schema.Properties[notebookAppendArg].Type)
	assert.Equal(t, false, schema.Properties[notebookAppendArg].Default)
